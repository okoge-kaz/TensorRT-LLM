...
JUDGE_PROMPT = "[Instruction]\n：あなたは、プロの翻訳家です。以下の英語の文章を日本語に翻訳してください。直訳はせず、日本語として適切な表現を利用して翻訳してください。翻訳した訳文だけを表示するべきであり、余計な表現は出力しないでください。また、ソースコード、数式など英語で記すべきものは翻訳しないでください。しかし、コード中のコメントなどは日本語に翻訳してください。では、次の英語の文章を日本語に翻訳してください。\n[English text]:"
...
def process_batch(model, tokenizer, batch, args):
    ...
    for item in batch:
        judged_text: str = JUDGE_PROMPT + item["instruction"]
    ...
    item["translated_instruction"] = output_text

    batch_input_ids = []
    input_lengths = []

    text = JUDGE_PROMPT + item["response"]
    input_text: str = tokenizer.apply_chat_template(
    ...
    item["translated_response"] = output_text
