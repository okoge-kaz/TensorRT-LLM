...
from examples.utils import add_common_args, load_tokenizer, read_model_name
...
def main(args) -> None:
    ...
    model = runner_cls.from_dir(args.engine_dir, rank=runtime_rank, **runner_kwargs)
    ...
    for idx, item in enumerate(data[start_index:], start=start_index + 1):
        ...
        # tensorRT
        inputs = tokenizer.encode(input, return_tensors="pt", add_special_tokens=False).squeeze(0)  # type: ignore
        ...
        with torch.no_grad():
            if isinstance(model, torch.nn.Module):
                ...
                paddings = [torch.ones(max_length - length, dtype=torch.int32) * pad_id for length in input_lengths]
                ...
                with torch.no_grad():
                    ...
                    outputs = model.generate(batch_input_ids, max_new_tokens=output_len, top_k=top_k)  # type: ignore
